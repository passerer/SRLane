...
__device__ inline bool devIoU(scalar_t const * const a, scalar_t const * const b, const float threshold) {
  const int start_a = (int) (a[2] + 0.5); // 0.5 rounding trick
  const int start_b = (int) (b[2] + 0.5);
  const int start = max(start_a, start_b);
  const int end_a = (int) (a[3] + 0.5);
  const int end_b = (int) (b[3] + 0.5);
  const int end = min(min(end_a, end_b), N_OFFSETS - 1);
  // if (end < start) return 1e9;
  if (end < start) return false;
  scalar_t dist = 0;
  for(unsigned char i = 4 + start; i <= 4 + end; ++i) {
    if (a[i] < b[i]) {
      dist += b[i] - a[i];
    } else {
      dist += a[i] - b[i];
