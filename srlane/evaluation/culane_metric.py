...
def load_culane_data(data_dir, file_list_path, load_size=False):
    ...
    if load_size:
        with open(file_list_path, 'r') as file_list:
            img_paths = [
                osp.join(data_dir, line[1 if line[0] == '/' else 0:].rstrip())
                for line in file_list.readlines()
            ]
            ori_img_shape = [
...
def eval_predictions(pred_dir,
    ...
    predictions, _ = load_culane_data(pred_dir, list_path, load_size=False)
    annotations, ori_img_shape = load_culane_data(anno_dir, list_path,
